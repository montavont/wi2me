...
def plot(config):
	...
	for source in config['data']:
                ...
                for i in range(buckets):
                    ntraffic = source.getTotalTransferredBytes(maxts = i * duration * 1000 / buckets )#0
                    ntraffic -= lastTraf
                    print ntraffic
                    lastTraf += ntraffic
                    tsum += ntraffic
                ...
                """for i in bucketSpeeds:
                    s = np.sum(bucketSpeeds[i])
                    inte = np.average(bucketInterv[i])
                    print str(s) + "    " + str(inte)"""
        ...
        if len(AvgThroughputs) > 0:
                ...
                AssociationAverage_bars = [float(Traffic[k]) / L2AssociationTimes[k] for k in sorted(AvgThroughputs)]
